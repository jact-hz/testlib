...
public class JazzyViewPager extends ViewPager {
    ...
    public JazzyViewPager(Context context, AttributeSet attrs) {
        ...
        TypedArray ta =
                context.obtainStyledAttributes(attrs, MResource.getIdsByName(
                        BaseInit.ctx.getPackageName(), "styleable", "JazzyViewPager"));
        int effect =
                ta.getInt(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                        "JazzyViewPager_style"), 0);
        String[] transitions =
                getResources().getStringArray(
                        MResource.getIdByName(BaseInit.ctx.getPackageName(), "array",
                                "jazzy_effects"));
        setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
        setFadeEnabled(ta.getBoolean(MResource.getIdByName(BaseInit.ctx.getPackageName(),
                "styleable", "JazzyViewPager_fadeEnabled"), false));
        setOutlineEnabled(ta.getBoolean(MResource.getIdByName(BaseInit.ctx.getPackageName(),
                "styleable", "JazzyViewPager_outlineEnabled"), false));
        setOutlineColor(ta.getColor(MResource.getIdByName(BaseInit.ctx.getPackageName(),
                "styleable", "JazzyViewPager_outlineColor"), Color.WHITE));
        switch (mEffect) {
            case Stack:
            case ZoomOut:
                setFadeEnabled(true);
    ...
    protected void animateTablet(View left, View right, float positionOffset) {
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                mTrans =
                        getOffsetXForRotation(mRot, left.getMeasuredWidth(),
            ...
            if (right != null) {
                ...
                mTrans =
                        getOffsetXForRotation(mRot, right.getMeasuredWidth(),
    ...
    private void animateZoom(View left, View right, float positionOffset, boolean in) {
        if (mState != State.IDLE) {
            if (left != null) {
                manageLayer(left, true);
                mScale =
                        in ? ZOOM_MAX + (1 - ZOOM_MAX) * (1 - positionOffset) : 1 + ZOOM_MAX
            ...
            if (right != null) {
                manageLayer(right, true);
                mScale =
                        in ? ZOOM_MAX + (1 - ZOOM_MAX) * positionOffset : 1 + ZOOM_MAX - ZOOM_MAX
                                * positionOffset;
                ViewHelper.setPivotX(right, right.getMeasuredWidth() * 0.5f);
                ViewHelper.setPivotY(right, right.getMeasuredHeight() * 0.5f);
    ...
    private void animateFlipHorizontal(View left, View right, float positionOffset,
    ...
    private void animateFlipVertical(View left, View right, float positionOffset,
            int positionOffsetPixels) {
        if (mState != State.IDLE) {
            if (left != null) {
                manageLayer(left, true);
