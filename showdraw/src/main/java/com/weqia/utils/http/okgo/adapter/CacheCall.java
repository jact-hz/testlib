...
import com.weqia.utils.http.okgo.OkGoException;
import com.weqia.utils.http.okgo.cache.CacheEntity;
import com.weqia.utils.http.okgo.cache.CacheManager;
import com.weqia.utils.http.okgo.cache.CacheMode;
import com.weqia.utils.http.okgo.callback.AbsCallbackWrapper;
...
import com.weqia.utils.http.okgo.utils.HeaderParser;
...
public class CacheCall<T> implements Call<T> {
    ...
    public void execute(RequestCallBack<T> callback) {
        ...
        if (baseRequest.getCacheKey() == null)
            baseRequest.setCacheKey(ParamsUtils.createUrlFromParams(baseRequest.getBaseUrl(), baseRequest.getParams().urlParams));
        ...
        if (cacheMode != CacheMode.NO_CACHE) {
            ...
            cacheEntity = CacheManager.getInstance().get(baseRequest.getCacheKey());
        ...
        if (cacheMode == CacheMode.IF_NONE_CACHE_REQUEST) {
            ...
            if (cacheEntity != null && !cacheEntity.isIsExpire()) {
                T data = (T) cacheEntity.getData();
    ...
    private void handleCache(Headers headers, T data) {
        ...
        if (cache == null) {
            //服务器不需要缓存，移除本地缓存
            CacheManager.getInstance().remove(baseRequest.getCacheKey());
        } else {
            //缓存命中，更新缓存
            CacheManager.getInstance().replace(baseRequest.getCacheKey(), (CacheEntity<Object>) cache);
        }
    }
        ...
        OkGo.getInstance().getDelivery().post(new Runnable() {
            ...
            public void run() {
                if (isFromCache) {
                    ...
                    mCallback.onSuccessData(t, call, response);      //请求成功回调 （UI线程）
                    mCallback.onAfter(t, null);                  //请求结束回调 （UI线程）
                }
            }
        });
