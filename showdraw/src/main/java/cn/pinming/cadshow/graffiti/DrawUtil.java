...
import static common.GlobalConstants.CLOUD_LINE_DIA;
import static common.GlobalConstants.CLOUD_LINE_RADIUS;
...
public class DrawUtil {
     ...
     * 画云线
     * ***/
    public static void drawRectWavy(Canvas canvas, float sx, float sy, float dx, float dy, Paint paint) {
        Path path = new Path();
        float endx = 0, endy = 0;          //为了让波浪线不超过边界,取波浪线结束的那个点坐标
        path.moveTo(sx, sy);
        if (dx - sx > 0) {
            if (dy - sy > 0) {
                for (int i = 0; i < dx - sx; i += CLOUD_LINE_DIA) {
                    //相对绘制二阶贝塞尔曲线(相对于自己的起始点--也即是上一个曲线的终点
                    path.quadTo(sx + i + CLOUD_LINE_RADIUS, sy - CLOUD_LINE_RADIUS, sx + i + CLOUD_LINE_DIA, sy);
                    endx = sx + i + CLOUD_LINE_DIA;
                }
            } else {
                for (int i = 0; i < dx - sx; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx + i + CLOUD_LINE_RADIUS, sy + CLOUD_LINE_RADIUS, sx + i + CLOUD_LINE_DIA, sy);
            ...
            if (dy - sy > 0) {
                for (int i = 0; i < sx - dx; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx - i - CLOUD_LINE_RADIUS, sy - CLOUD_LINE_RADIUS, sx - i - CLOUD_LINE_DIA, sy);
                    endx = sx - i - CLOUD_LINE_DIA;
                }
            } else {
        ...
        if (dy - sy > 0) {
            if (dx - sx > 0) {
                for (int i = 0; i < dy - sy; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx - CLOUD_LINE_RADIUS, sy + i + CLOUD_LINE_RADIUS, sx, sy + i + CLOUD_LINE_DIA);
                    endy = sy + i + CLOUD_LINE_DIA;
                }
            } else {
                for (int i = 0; i < dy - sy; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx + CLOUD_LINE_RADIUS, sy + i + CLOUD_LINE_RADIUS, sx, sy + i + CLOUD_LINE_DIA);
            ...
            if (dx - sx > 0) {
                for (int i = 0; i < sy - dy; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx - CLOUD_LINE_RADIUS, sy - i - CLOUD_LINE_RADIUS, sx, sy - i - CLOUD_LINE_DIA);
                    endy = sy - i - CLOUD_LINE_DIA;
                }
            } else {
        ...
        if (dx - sx > 0) {
            if (dy - sy > 0) {
                for (int i = 0; i < dx - sx; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx + i + CLOUD_LINE_RADIUS, endy + CLOUD_LINE_RADIUS, sx + i + CLOUD_LINE_DIA, endy);
                }
            } else {
                for (int i = 0; i < dx - sx; i += CLOUD_LINE_DIA) {
                    path.quadTo(sx + i + CLOUD_LINE_RADIUS, endy - CLOUD_LINE_RADIUS, sx + i + CLOUD_LINE_DIA, endy);
        ...
        if (dy - sy > 0) {
            if (dx - sx > 0) {
                for (int i = 0; i < dy - sy; i += CLOUD_LINE_DIA) {
                    path.quadTo(endx + CLOUD_LINE_RADIUS, sy + i + CLOUD_LINE_RADIUS, endx, sy + i + CLOUD_LINE_DIA);
                }
            } else {
                for (int i = 0; i < dy - sy; i += CLOUD_LINE_DIA) {
                    path.quadTo(endx - CLOUD_LINE_RADIUS, sy + i + CLOUD_LINE_RADIUS, endx, sy + i + CLOUD_LINE_DIA);
