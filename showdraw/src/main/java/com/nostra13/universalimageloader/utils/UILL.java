 ...
 * @since 1.6.4
 */
public final class UILL {

    private static final String LOG_FORMAT = "%1$s\n%2$s";
    private static volatile boolean writeDebugLogs = BitmapInit.DEBUG_BITMAP;
    private static volatile boolean writeLogs = BitmapInit.DEBUG_BITMAP;
    ...
    private static void log(int priority, Throwable ex, String message, Object... args) {
        if (!writeLogs) return;
        String log = getMsgObj(ex, message, args);
//        if (priority == Log.ERROR) {
            Log.println(priority, "WQ:", log);
//        }
    }

    private static String getMsgObj(Throwable ex, String message, Object... args) {
        if (args.length > 0) {
