...
//            dialog = DialogUtil.commonLoadingDialog(ctx, loadingStr);
    ...
    public void onSuccess(String content) {
        ...
        if (StrUtil.isEmptyOrNull(content)) {
            ...
            onError(EnumData.ErrorCodeType.RECEIVE_NOTHING.order());
            return;
        }
        if (L.D) {
...
//                    }
                } else if (retNo.intValue() == EnumData.ErrorCodeType.USER_IS_LEAVE.order()) {
                    // 数据库清除
                    WeqiaApplication.isLev = true;
                    EventBus.getDefault().post(new RefreshEvent(LEAVE_COMPANY));
                    return;
                } else if (retNo.intValue() == EnumData.ErrorCodeType.SIGN_VALID_ERROR.order()) {
...
//                                        }
            ...
            if (resultEx == null) {
                onError(EnumData.ErrorCodeType.ILLEGAL_DATA_FORMAT.order());
                return;
            } else {
...
/*            String ret = resultEx.getRet(); // 代表数量
            ...
            if (resultEx.getSuccess()) {
                ...
                onError(EnumData.ErrorCodeType.RET_NULL.order());
            }
        } catch (NumberFormatException e) {
            // if (L.D) {
            // L.e(ErrorCodeType.REL_ILLEGAL.strName());
            // }
            onError(EnumData.ErrorCodeType.REL_ILLEGAL.order());
            CheckedExceptionHandler.handleException(e);
        }
    }
...
//            HttpUtil.getInstance().closeAllConnect();
        ...
        if (StrUtil.notEmptyOrNull(strMsg)) {
            if (strMsg.contains("error code:500")) {
                onError(EnumData.ErrorCodeType.CLIENT_NOT_CONNECT.order());
            } else if (strMsg.equalsIgnoreCase(UtilsConstants.NETWORK_MSG)) {
                onError(EnumData.ErrorCodeType.NETWORK_ERROR_NEW.order());
            } else {
//
...
//                }
                ...
                onError(EnumData.ErrorCodeType.CLIENT_SYS_ERROR.order());
            }
        } else {
            onError(EnumData.ErrorCodeType.NETWORK_ERROR.order());
        }
...
//            ctx = WeqiaApplication.ctx;
        ...
        if (ctx != null) {
            EnumData.ErrorCodeType type = EnumData.ErrorCodeType.valueOf(errCode);
            if (type != null) {
                if (type.order() == EnumData.ErrorCodeType.CLIENT_NOT_CONNECT.order()) {
                    L.d(EnumData.ErrorCodeType.CLIENT_NOT_CONNECT.strName());
                } else if (type.order() == EnumData.ErrorCodeType.NETWORK_ERROR.order()) {
                    // L.toastShort(ErrorCodeType.NETWORK_ERROR.strName());
                } else if (type.order() == EnumData.ErrorCodeType.CLIENT_SYS_ERROR.order()) {
                    // try {
                    // // 显示错误消息对话框
                ...
                } else if (type.order() == EnumData.ErrorCodeType.NETWORK_ERROR_NEW.order()) {
