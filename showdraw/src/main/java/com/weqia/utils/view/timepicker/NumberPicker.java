 ...
 * <ul>
 * <li>
 * If the current theme is derived from {@link android.R.style#Theme} the widget presents the
 * current value as an editable input field with an increment button above and a decrement button
 * below. Long pressing the buttons allows for a quick change of the current value. Tapping on the
 * input field allows to type in a desired value.</li>
 * <li>
 * If the current theme is derived from {@link android.R.style#Theme_Holo} or
 * {@link android.R.style#Theme_Holo_Light} the widget presents the current value as an editable
 * input field with a lesser value above and a greater value below. Tapping on the lesser or greater
 * value selects it by animating the number axis up or down to make the chosen value current.
 * Flinging up or down allows for multiple increments or decrements of the current value. Long
 * pressing on the lesser and greater values also allows for a quick change of the current value.
 * Tapping on the current value allows to type in a desired value.</li>
 * </ul>
 * <p>
...
public class NumberPicker extends LinearLayout {
     ...
     */
    private static final int SELECTOR_WHEEL_ITEM_COUNT = 2;
    ...
    public static final NumberPicker.Formatter DAY_FORMATTER = new NumberPicker.Formatter() {
    ...
    public NumberPicker(Context context, AttributeSet attrs) {
        this(context, attrs, MResource.getIdByName(BaseInit.ctx.getPackageName(), "attr",
                "numberPickerStyle"));
    }
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray attributesArray =
                context.obtainStyledAttributes(attrs, MResource.getIdsByName(
                        BaseInit.ctx.getPackageName(), "styleable", "NumberPicker"), defStyle, 0);

        mHasSelectorWheel = true;// (layoutResId != DEFAULT_LAYOUT_RESOURCE_ID);
        mSolidColor =
                attributesArray.getColor(MResource.getIdByName(BaseInit.ctx.getPackageName(),
                        "styleable", "NumberPicker_solidColor"), 0);
        mSelectionDivider =
                attributesArray.getDrawable(MResource.getIdByName(BaseInit.ctx.getPackageName(),
                        "styleable", "NumberPicker_selectionDivider"));
        final int defSelectionDividerHeight =
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                        UNSCALED_DEFAULT_SELECTION_DIVIDER_HEIGHT, getResources()
                                .getDisplayMetrics());
        mSelectionDividerHeight =
                attributesArray.getDimensionPixelSize(MResource.getIdByName(
                        BaseInit.ctx.getPackageName(), "styleable",
                        "NumberPicker_selectionDividerHeight"), defSelectionDividerHeight);
        final int defSelectionDividerDistance =
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                        UNSCALED_DEFAULT_SELECTION_DIVIDERS_DISTANCE, getResources()
                                .getDisplayMetrics());
        mSelectionDividersDistance =
                attributesArray.getDimensionPixelSize(MResource.getIdByName(
                        BaseInit.ctx.getPackageName(), "styleable",
                        "NumberPicker_selectionDividersDistance"), defSelectionDividerDistance);

        mMinHeight =
                attributesArray.getDimensionPixelSize(MResource.getIdByName(
                        BaseInit.ctx.getPackageName(), "styleable",
                        "NumberPicker_android_minHeight"), SIZE_UNSPECIFIED);

        mMaxHeight =
                attributesArray.getDimensionPixelSize(MResource.getIdByName(
                        BaseInit.ctx.getPackageName(), "styleable",
                        "NumberPicker_android_maxHeight"), SIZE_UNSPECIFIED);
        ...
        mMinWidth =
                attributesArray.getDimensionPixelSize(
                        MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                                "NumberPicker_android_minWidth"), SIZE_UNSPECIFIED);

        mMaxWidth =
                attributesArray.getDimensionPixelSize(
                        MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                                "NumberPicker_android_maxWidth"), SIZE_UNSPECIFIED);
        ...
        inflater.inflate(MResource.getIdByName(BaseInit.ctx.getPackageName(), "layout",
                "util_number_picker"), this, true);

        OnClickListener onClickListener = new OnClickListener() {
            public void onClick(View v) {
                hideSoftInput();
                mInputText.clearFocus();
                if (v.getId() == MResource.getIdByName(BaseInit.ctx.getPackageName(), "id",
                        "increment")) {
        ...
        if (!mHasSelectorWheel) {
            mIncrementButton =
                    (ImageButton) findViewById(MResource.getIdByName(BaseInit.ctx.getPackageName(),
                            "id", "increment"));
        ...
        // input text
        mInputText =
                (EditText) findViewById(MResource.getIdByName(BaseInit.ctx.getPackageName(), "id",
                        "numberpicker_input"));
...
//        });
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) {
            ...
            mTopSelectionDividerTop =
                    (getHeight() - mSelectionDividersDistance) / 2 - mSelectionDividerHeight;
            mBottomSelectionDividerBottom =
                    mTopSelectionDividerTop + 2 * mSelectionDividerHeight
                            + mSelectionDividersDistance;
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // min.
        final int widthSize =
                resolveSizeAndStateRespectingMinSize(mMinWidth, getMeasuredWidth(),
                        widthMeasureSpec);
        final int heightSize =
                resolveSizeAndStateRespectingMinSize(mMinHeight, getMeasuredHeight(),
                        heightMeasureSpec);
    ...
    /*
     * @Override protected boolean dispatchHoverEvent(MotionEvent event) { if (!mHasSelectorWheel) {
     * return super.dispatchHoverEvent(event); } if
     * (AccessibilityManager.getInstance(mContext).isEnabled()) { final int eventY = (int)
     * event.getY(); final int hoveredVirtualViewId; if (eventY < mTopSelectionDividerTop) {
     * hoveredVirtualViewId = AccessibilityNodeProviderImpl.VIRTUAL_VIEW_ID_DECREMENT; } else if
     * (eventY > mBottomSelectionDividerBottom) { hoveredVirtualViewId =
     * AccessibilityNodeProviderImpl.VIRTUAL_VIEW_ID_INCREMENT; } else { hoveredVirtualViewId =
     ...
     * (AccessibilityNodeProviderImpl) getAccessibilityNodeProvider(); switch (action) { case
     ...
     * MotionEvent.ACTION_HOVER_MOVE: { if (mLastHoveredChildVirtualViewId != hoveredVirtualViewId
     ...
     * provider.sendAccessibilityEventForVirtualView(hoveredVirtualViewId,
     * AccessibilityEvent.TYPE_VIEW_HOVER_EXIT); mLastHoveredChildVirtualViewId = View.NO_ID; }
     * break; } } return false; }
     */
    @Override
     ...
     * {@link NumberPicker#getWrapSelectorWheel()} is <code>false</code> the current value is set to
     * the {@link NumberPicker#getMinValue()} value.
     * </p>
     * <p>
     * If the argument is less than the {@link NumberPicker#getMinValue()} and
     * {@link NumberPicker#getWrapSelectorWheel()} is <code>true</code> the current value is set to
     * the {@link NumberPicker#getMaxValue()} value.
     * </p>
     * <p>
     ...
     * {@link NumberPicker#getMinValue()} and {@link NumberPicker#getMaxValue()} values.
     * <p>
     ...
     * </p>
     * <p>
     * <strong>Note:</strong> If the number of items, i.e. the range ( {@link #getMaxValue()} -
     * {@link #getMinValue()}) is less than the number of items shown on the selector wheel, the
     * selector wheel will not wrap. Hence, in such a case calling this method is a NOP.
     * </p>
     * 
    ...
    private boolean updateInputTextView() {
         ...
         */
        String text =
                (mDisplayedValues == null) ? formatNumber(mValue) : mDisplayedValues[mValue
                        - mMinValue];
    ...
    class InputTextFilter extends NumberKeyListener {
        ...
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest,
            ...
            if (mDisplayedValues == null) {
                ...
                /*
                 * Ensure the user can't type in a value greater than the max allowed. We have to
                 * allow less than min as the user might want to delete some numbers and then type a
                 * new number.
                 */
                if (val > mMaxValue) {
                    return "";
