...
import com.weqia.utils.view.pullrefresh.internal.FlipLoadingLayout;
import com.weqia.utils.view.pullrefresh.internal.LoadingLayout;
import com.weqia.utils.view.pullrefresh.internal.RotateLoadingLayout;
import com.weqia.utils.view.pullrefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
     ...
     * @deprecated This simple calls setMode with an appropriate mode based on the passed value.
     */
     ...
     * @param attrs AttributeSet from wrapped class. Means that anything you include in the XML
     *        layout declaration will be routed to the created View
    ...
    protected final int getHeaderSize() {
        int headSize = mHeaderLayout.getContentSize();
        return headSize;
    }
    ...
    protected final void refreshRefreshableViewSize(int width, int height) {
        ...
        // this layout
        LayoutParams lp =
                (LayoutParams) mRefreshableViewWrapper.getLayoutParams();
    ...
    protected final void setHeaderScroll(int value) {
        // if (L.D) L.e("setHeaderScroll = " + value);
        // if (DEBUG) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray a =
                context.obtainStyledAttributes(attrs, MResource.getIdsByName(
                        BaseInit.ctx.getPackageName(), "styleable", "PullToRefresh"));

        if (a.hasValue(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                "PullToRefresh_ptrMode"))) {
            mMode =
                    Mode.mapIntToValue(a.getInteger(MResource.getIdByName(
                            BaseInit.ctx.getPackageName(), "styleable", "PullToRefresh_ptrMode"), 0));
        }

        if (a.hasValue(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                "PullToRefresh_ptrAnimationStyle"))) {
            mLoadingAnimationStyle =
                    AnimationStyle.mapIntToValue(a.getInteger(MResource.getIdByName(
        ...
        if (a.hasValue(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
            ...
            Drawable background =
                    a.getDrawable(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                            "PullToRefresh_ptrRefreshableViewBackground"));
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
        ...
        if (a.hasValue(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
            ...
            mOverScrollEnabled =
                    a.getBoolean(MResource.getIdByName(BaseInit.ctx.getPackageName(), "styleable",
                            "PullToRefresh_ptrOverScroll"), true);
        }
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            case PULL_FROM_END:
                newScrollValue =
                        Math.round(Math.max(initialMotionValue - lastMotionValue, 0) / FRICTION);
                itemDimension = getFooterSize();
                break;
            case PULL_FROM_START:
            default:
                newScrollValue =
                        Math.round(Math.min(initialMotionValue - lastMotionValue, 0) / FRICTION);
    ...
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END}
         *        depending on which direction the user is pulling. Only useful when
    ...
    /**
     * An advanced version of the Listener to listen for callbacks to Refresh. This listener is
     * different as it allows you to differentiate between Pull Ups, and Pull Downs.
     * 
     * @author Chris Banes
     */
    public static interface OnRefreshListenerBoth<V extends View> {
        ...
        /**
         * onPullDownToRefresh will be called only when the user has Pulled from the start, and
         * released.
         */
        public void onPullDownToRefresh(final PullToRefreshBase<V> refreshView);

        /**
         * onPullUpToRefresh will be called only when the user has Pulled from the end, and
         * released.
         */
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mStartTime == -1) {
                 ...
                 */
                long normalizedTime =
                        (1000 * (System.currentTimeMillis() - mStartTime)) / mDuration;
