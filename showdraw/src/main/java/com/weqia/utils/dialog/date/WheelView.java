package com.weqia.utils.dialog.date;
...
public class WheelView extends View {
    ...
    private List<OnWheelClickedListener> clickingListeners =
            new LinkedList<OnWheelClickedListener>();
    ...
    /**
     * Sets the desired count of visible items. Actual amount of visible items depends on wheel
     * layout parameters. To apply changes and rebuild view call measure().
     * 
    ...
    public void setCurrentItem(int index, boolean animated) {
        ...
        if (index != currentItem) {
            if (animated) {
                ...
                if (isCyclic) {
                    int scroll =
                            itemCount + Math.min(index, currentItem) - Math.max(index, currentItem);
    ...
    private void initResourcesIfNecessary() {
        if (centerDrawable == null) {
            centerDrawable =
                    getContext().getResources().getDrawable(
                            MResource.getIdByName(BaseInit.ctx.getPackageName(), "drawable",
                                    "util_picker_bar"));
        }
        ...
        // setBackgroundResource(R.drawable.util_wheel_bg);
    }
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        itemsLayout.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, 50));
